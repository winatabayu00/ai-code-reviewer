...
If no issues: {"decision":"APPROVE","issues":[]}`
  ...
  let modelType = process.env.AI_MODEL_TYPE
  if (!modelType) {
    console.warn("⚠️ AI_MODEL_TYPE not set, defaulting to 'deepseek'")
    modelType = "deepseek"
  }
  const supportedModels = ["deepseek", "nvidia", "openai"]
  if (!supportedModels.includes(modelType)) {
    console.warn(`⚠️ Unknown AI_MODEL_TYPE: ${modelType}, defaulting to 'deepseek'`)
    modelType = "deepseek"
  }
